...
module.exports = class Backend
{
  ...
  async userShouldUpdateXP( user )
  {
    const key = ['userlastxppush', user.id].join( '_' )
    const prevtime = await this.redisGet( key ) || 0
    const delta = ( moment().unix() - prevtime )
  ...
  async userAddXP( dsUser, dsGuildMember, xp )
  {
    ...
    const gxp = await this.redisIncrementFloat( gkey, xp )
    if ( shouldUpdateDB )
    {
    ...
    if ( skey && guild )
    {
      const sxp = await this.redisIncrementFloat( skey, xp )
      if ( shouldUpdateDB )
      {
